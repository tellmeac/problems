...
// KolyaSolution задача про агрегацию потока метрик обработку последовательных команд за эффективное время.
func KolyaSolution(r io.Reader, writer io.Writer) {
	w := bufio.NewWriter(writer)

	var nodeC, serversC, eventsC int
	_, _ = fmt.Fscanf(r, "%d %d %d\n", &nodeC, &serversC, &eventsC)
	...
	for ; eventsC > 0; eventsC-- {
		...
		case "GETMAX":
			...
			_, _ = fmt.Fscanf(r, "\n")
			_, _ = fmt.Fprintln(w, maxAns.Node.ID)
		case "GETMIN":
			if minAns.Dirty {
				// update
				minAns.Node = nodes[0]
			...
			_, _ = fmt.Fscanf(r, "\n")
			_, _ = fmt.Fprintln(w, minAns.Node.ID)
		default:
